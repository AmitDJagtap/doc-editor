...
window.Wodo = window.Wodo || (function () {
            ...
            "webodf/editor/Translator"],
                function (BC, CP, FWZH, ES, T, Translator) {
                    ...
                    t = new Translator(translationsDir, locale, function (editorTranslator) {
                        ...
                        isInitalized = true;
                        pendingInstanceCreationCalls.forEach(function (create) {
                            create();
                        });
                    });
    ...
    function TextEditor(mainContainerElementId, editorOptions) {
        ...
        /**
         * This function adds a table to document with given form data array..in format [["gender","male"],["name":"xyz"]] ..
         * number of rows is auto calculated and columns based on size of array
         * @param {Array[rows][column]} formData
         * @returns {odfCanvas}
         */
        this.insertTable = function (formData) {
            var numColumns = formData[0].length;
            var numRows = formData.length;
            var _base_style_name= "Table8.A";
            var items = [];
            
            for (var i = 0; i < numRows; i++) {
                var styleName = _base_style_name + 1;
                var columnStyles = [];
                for (var j = 0; j < numColumns; j++) {
                    columnStyles.push(styleName);
                }
                items.push(columnStyles);
            }
            //Call Create Table
            editorSession.insertTable(numRows, numColumns, "Table8", "Table8.A", items, formData);
        };
        /**
         * This function adds a text to document with selected data.
         * @param {String} text
         * @returns {odfCanvas}
         */
         this.insertText = function(text) {
             var textController = editorSession.sessionController.getTextController();
